...
(function ($) {
	...
	$.fn.fxdHdrCol = function (o) {
		...
		return this.each (function () {
			...
			//************************ */
			//Eason Create
			//如果没有数据，td的自适应宽度会导致几个table的长度不一致
			lc.ft_r.append('<tbody />');
				
			//append row by row while just keeping the frozen cols
			var ftr_tbody = lc.ft_r.find('tbody'); 
			$.each ($this.find('tbody > tr'), function (idx, el) {
				var tr = $(el).clone();	
				ftr_tbody.append(tr);
		
			});
			ftr_tbody.css('visibility', 'hidden');
			lc.ft_r.css('display', 'block');
			lc.ft_r.css('height', '50px')
			//************************ */
